...
# XXD Panel 187｜كولاج أرشيفي بحروف كبيرة
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 187 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-187`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-187 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-187-prompt.en.md) أو [بالصينية](references/xxd-panel-187-prompt.zh-CN.md).
